...
def gather(target: str, k8s, k8s_apps, k8s_auth, k8s_rbac,
           on_step=None) -> str:
    """
    ...
    # ── Always run ────────────────────────────────────────────────────────────
    run("PERMISSIONS (what can this identity do?)",
        list_permissions, k8s_auth, namespace=namespace)

    run("SERVICE ACCOUNTS + BINDINGS",
        list_serviceaccounts, k8s, k8s_rbac, namespace=namespace)
    ...
    run("CLUSTER ROLES",
        list_clusterroles, k8s_rbac)

    # ── Pod-specific ──────────────────────────────────────────────────────────
    if kind == "pod":
        run(f"TARGET POD ({namespace}/{name})",
            list_pods, k8s, k8s_rbac=k8s_rbac,
            namespace=namespace, show_sa=True)

        run("SECRETS ACCESSIBLE",
            list_secrets, k8s, k8s_apps=k8s_apps,
            namespace=namespace, pod=name)

    # ── Namespace-specific ────────────────────────────────────────────────────
    if kind in ("namespace", "pod"):
        run("ALL PODS IN SCOPE",
            list_pods, k8s, k8s_rbac=k8s_rbac,
            namespace=namespace, show_sa=True)

        run("SECRETS IN NAMESPACE",
            list_secrets, k8s, k8s_apps=k8s_apps,
            namespace=namespace)

    # ── SA-specific ───────────────────────────────────────────────────────────
    if kind == "sa":
        run(f"TARGET SERVICE ACCOUNT ({namespace}/{name})",
            list_serviceaccounts, k8s, k8s_rbac, namespace=namespace)

        run("PODS RUNNING AS THIS SA",
            list_pods, k8s, k8s_rbac=k8s_rbac,
            namespace=namespace, show_sa=True)

    # ── Security checks — always ──────────────────────────────────────────────
    run("PRIVILEGED CONTAINERS",
        check_privileged_pods, k8s, k8s_apps, namespace=namespace)

    run("ROOT CONTAINERS",
        check_root_containers, k8s, k8s_apps=k8s_apps, namespace=namespace)
    ...
    run("SECRET HARVEST — decoded credentials",
        harvest_secrets, k8s, namespace=namespace)
